...
if (config.server.update) {
    requester(config.server.link, (err, req, data) => {
        ...
        if (config.server.version < requesterConfig.server.version) {
            logger.warn(`[Information] A new update was found!`)
            logger.warn(`[Information] Download -> https://github.com/GeniusXD/free-agario-fb-bots`)
			process.exit();
        } else {
            logger.good(`[Information] No updates found!`)
        }
    })
} else {
    logger.error('[Information] Update is false!')
}
...
logger.good(`===============================================`)
logger.good(`[Data] Modification Date : Friday 27 June 2020`)
logger.good(`[Data] Author by : M4N4T0R0`)
logger.good(`[Data] Fork from : Nels Old Bot`)
logger.good(`===============================================`)
logger.good(`[Version] Running version ${config.server.version} on port ${config.server.port}`)
...
const dataBot = {
    ...
    onclose() {
        if (this.isConnected) {
            this.isConnected = false
            this.connect()
            logger.error('[Information] DataBot disconnected!')
        }
    },
    handleBuffer(buffer) {
        ...
        switch (reader.readUint8()) {
            ...
            case 241:
                ...
                logger.good('[Information] DataBot connected!')
                break
        }
    }
}
            ...
            case 32:
                ...
                if (!this.isAlive) {
                    ...
                    if (!user.startedBots) {
                        ...
                        logger.good('[Information] The Bot is Now Started!')
                    }
...
new WebSocket.Server({
    ...
    userWS = ws
    logger.good('[Information] The User is Now Connected!')
        ...
        switch (reader.readUint8()) {
            case 0:
                if (!user.startedBots) {
                    ...
                    logger.good('[Information] The Bot is Starting ...')
                    logger.good("[Information] Remaining Captcha Tokens:", user.tokens.length);
                }
                break
            case 1:
                if (user.startedBots && !stoppingBots) {
                    ...
                    setInterval(() => {
                        if (seconds === 30) {
                            ...
                            logger.warn(`[Information] The Bot is Now Stopping in ${30 - seconds} seconds`)
                            seconds++
                        }
                    }, 1000)
                }
    ...
    ws.on('close', () => {
        ...
        logger.error('[Information] The User is Now disconnected!')
    })
})
